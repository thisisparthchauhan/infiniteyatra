...
export default function Destinations({
    ...
    const displayPackages = propPackages ?? [];
            ...
            <div className="container mx-auto px-6">
                {!disableHeader && (
                    <div className="flex items-end justify-between mb-12">
                        ...
                        {showViewAll && (
                            <Link href="/tours" className="hidden md:flex items-center gap-2 text-blue-400 font-medium hover:text-blue-300 transition-colors">
                ...
                {displayPackages.length > 0 ? (
                                ...
                                <div className={`${cardClass} p-3 h-full`}>
                                    <div className="relative aspect-[4/5] overflow-hidden rounded-2xl mb-4">
                                        ...
                                        <div className="absolute top-4 right-4 flex gap-2 z-10">
                                            ...
                                            <div className="bg-black/40 backdrop-blur-md px-3 py-1 rounded-full flex items-center gap-1 text-sm font-medium text-white h-[34px] border border-white/10">
                                    ...
                                    <div className="space-y-2 px-1 pb-2">
                                        ...
                                        <div className="flex items-center justify-between">
                                            ...
                                            <span className={`font-bold text-lg ${textPrimary}`}>{dest.priceDisplay as string}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div className="text-center py-12">
                        <p className={`text-xl ${isLight ? 'text-slate-400' : 'text-white/50'}`}>No packages found.</p>
                    </div>
                )}

                {showViewAll && (
                    <div className="mt-12 md:hidden text-center">
                        <Link href="/tours" className={`glass-btn ${isLight ? '!bg-blue-500 !text-white' : ''}`}>
                            View all <ArrowRight size={20} />
                        </Link>
                    </div>
                )}
