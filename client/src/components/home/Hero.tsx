...
export default function Hero() {
    const [formData, setFormData] = useState({ location: '', days: '', persons: '', phone: '' });
    ...
    const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (name === 'location') {
            if (value.length > 0) {
                setSuggestions(popularDestinations.filter(city => city.toLowerCase().includes(value.toLowerCase())));
    ...
    return (
        <div className="relative h-screen min-h-[700px] flex items-center justify-center overflow-hidden">
            ...
            <div className="relative z-10 container mx-auto px-4 text-center text-white flex flex-col items-center">
                ...
                >
                    ...
                    <motion.div animate={{ scale: [1, 1.2, 1] }} transition={{ repeat: Infinity, duration: 2, ease: "easeInOut" }}>
                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="#ff4d4d" stroke="none"><path d="M19 14c1.49-1.46 3-3.21 3-5.5A5.5 5.5 0 0 0 16.5 3c-1.76 0-3 .5-4.5 2-1.5-1.5-2.74-2-4.5-2A5.5 5.5 0 0 0 2 8.5c0 2.3 1.5 4.05 3 5.5l7 7Z" /></svg>
                    </motion.div>
                </motion.div>
                            ...
                            >
                                    ...
                                    <div className="text-left w-full">
                                        ...
                                        <div className="relative">
                                            <input
                                                ...
                                                placeholder="Where to?" required autoComplete="off"
                                                    ...
                                                    >
                                                        ...
                                                        <div className="max-h-60 overflow-y-auto p-2">
                                                            {suggestions.slice(0, 8).map((city, idx) => (
                                                                <div
                                                                    key={idx}
                                                                    className="px-4 py-3 rounded-xl hover:bg-white/10 cursor-pointer flex items-center gap-4 transition-all group"
                                                                    onMouseDown={() => { setFormData(f => ({ ...f, location: city })); setShowSuggestions(false); }}
                                                                >
                                                                    ...
                                                                    <div className="flex-1 text-left">
                                                                        ...
                                                                        <p className="text-xs text-white/40">{city.split(',')[1]?.trim() || 'India'}</p>
                                                                    </div>
                                                                </div>
                                                            ))}
                                                        </div>
                                ...
                                <div className="flex-[0.7] flex items-center gap-3 px-6 py-4 w-full border-b lg:border-b-0 lg:border-r border-white/10">
                                    ...
                                    <div className="text-left w-full">
                                        ...
                                        <input id="days" name="days" type="number" min="1" value={formData.days} onChange={handleChange} placeholder="Days"
                                            className="w-full outline-none text-white placeholder:text-white/70 font-medium text-lg bg-transparent" />
                                    </div>
                                </div>
                                ...
                                <div className="flex-[0.7] flex items-center gap-3 px-6 py-4 w-full border-b lg:border-b-0 lg:border-r border-white/10">
                                    ...
                                    <div className="text-left w-full">
                                        ...
                                        <input id="persons" name="persons" type="number" min="1" value={formData.persons} onChange={handleChange} placeholder="Guests"
                                            className="w-full outline-none text-white placeholder:text-white/70 font-medium text-lg bg-transparent" />
                                    </div>
                                </div>
                                ...
                                <div className="flex-[1.8] flex items-center gap-3 px-6 py-4 w-full">
                                    ...
                                    <div className="text-left w-full">
                                        <label className="block text-[10px] text-white/70 font-bold uppercase tracking-wider mb-1">Phone</label>
                                        <div className="phone-input-dark w-full">
                                            <PhoneInput
                                                ...
                                                inputStyle={{ width: '100%', background: 'transparent', border: 'none', color: 'white', fontSize: '1.125rem', fontWeight: 500, paddingLeft: '48px' }}
                                ...
                                <div className="p-1 w-full lg:w-auto">
                                    <button type="submit" disabled={isSubmitting}
                                        className="bg-white text-black hover:bg-white/90 px-8 py-5 rounded-[1.7rem] w-full lg:w-auto transition-all font-bold text-lg shadow-lg flex items-center justify-center gap-2 min-w-[160px]">
                                        {isSubmitting ? <Loader size={20} className="animate-spin" /> : <><span>Plan Trip</span><Send size={18} /></>}
                                    </button>
                                </div>
                            </motion.form>
                        )}
            ...
            {/* Scroll cue */}
            <motion.div
                className="absolute bottom-10 left-1/2 -translate-x-1/2 z-20 opacity-50 hover:opacity-100 transition-opacity cursor-pointer"
